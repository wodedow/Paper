...
from modules.attention import AttentionModule, AttentionMergeMask
...
class ContentEnCoder(nn.Module):
    def __init__(self):
        ...
        self.conpconv1 = PConvLayer(3, 64, sample='down-3', activ='relu')
        ...
        for i in range(3):
            ...
            block = PConvLayer(64, 64, sample='down-4', activ='relu')
            setattr(self, conpconv, block)

        self.conpconv6 = PConvLayer(64, 128, sample='down-2', activ='relu')
    ...
    def forward(self, image, mask):
        ...
        x1, m1 = self.conpconv1(image, mask)

        x1, m1, x2, m2, att = self.conpconv2(x1, m1)  # Size(bs, 64, 128, 128)
        outputs.append(x1)
        masks.append(m1)
        outputs_.append(x2)
        masks_.append(m2)
        ...
        for i in range(4):
            conpconv = f'conpconv{i+3}'
            x1, m1 = getattr(self, conpconv)(x1, m1)

        x1, m1, x2, m2, att = self.conpconv7(x1, m1)  # Size(bs, 128, 64, 64)
        outputs.append(x1)
        masks.append(m1)
        outputs_.append(x2)
        masks_.append(m2)
        ...
        return outputs, masks, outputs_, masks_, att_scores
...
class ContentDeCoder(nn.Module):
    def __init__(self, transfer=True):
        ...
        if self.transfer:
            self.attention_transfer1 = AttentionMergeMask(128)
            self.attention_transfer2 = AttentionMergeMask(64)

    def forward(self, inputs, masks, att_scores=0.0):
        ...
        if self.transfer:
            x1 = self.attention_transfer1(
                x1, masks[-5], att_scores[-1])  # Size(bs, 128, 64, 64)
        ...
        if self.transfer:
            x1 = self.attention_transfer2(
                x1, masks[-6], att_scores[-2])  # Size(bs, 64, 128, 128)
...
class Main(BaseNetwork):
    ...
    def forward(self, image, mask):
        outputs, masks, outputs_, masks_, att_scores = self.encoder(
            image, mask)
        # print("process: encoder")

        x1, m1 = self.decoder(outputs, masks, att_scores)
