...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        try:
            ...
            print(f'can not load data[{index}]')
    ...
    def load_mask(self, img, index):
        ...
        if self.mask_type == 0:
            ...
            mask = self.resize(mask, aspect_ratio_kept=True)
        ...
        if self.mask_type == 1:
            mask = 1 - generate_stroke_mask(
                [self.target_size, self.target_size])
            mask = self.resize(mask, aspect_ratio_kept=False)
    ...
    def resize(self,
               ...
               centerCrop=False):
        '''
        Args: img, aspect_ratio_kept, fixed_size, centerCrop
        aspect_ratio_kept: keep ratio of img_size
        '''
        if img.shape[0] == self.target_size and img.shape[
                1] == self.target_size:
            img = np.array(Image.fromarray(img.astype('uint8')))
            return img
        ...
        img = np.array(
            Image.fromarray(img.astype('uint8')).resize(
                (self.target_size, self.target_size)))
        return img
    ...
    def load_list(self, path):
        if isinstance(path, str):
            if path[-3:] == 'txt':
                ...
                file_names = []
                for line in lines:
                    file_names.append('../../Dataset/Places2/train/data_256' +
                                      line.split(" ")[0])

                return file_names

            if os.path.isdir(path):
                path = list(glob.glob(path + '/*.jpg')) + list(
                    glob.glob(path + '/*.png'))
                path.sort()
                return path
...
def generate_stroke_mask(im_size,
    ...
    for i in range(parts):
        mask = mask + np_free_form_mask(maxVertex, maxLength, maxBrushWidth,
                                        maxAngle, im_size[0], im_size[1])
